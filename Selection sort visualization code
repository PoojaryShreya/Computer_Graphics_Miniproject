...
{   
    ...
    glEnd();
    glColor3f(1,0.2,0.5);
	bitmap_output(350, 530,"SELECTION SORT ALGORITHM ",GLUT_BITMAP_HELVETICA_18);//title larger font
    ...
    bitmap_output(10, 495, "Illustration of selection sort algorithm swapping using circles with varying radius",GLUT_BITMAP_HELVETICA_18);
   

	
	if (sorting == 0)//if not sorting display menu
	{
		bitmap_output(10, 458, "WELCOME",GLUT_BITMAP_HELVETICA_18);
...
{       int q;
		glutInit(&argc,argv);
		printf("ENTER THE TOTAL NUMBER OF ELEMENTS (between 1 to 11)");
        scanf("%d",&size);
        if(size>1 && size<=MAX)
        {
		    ...
		    glutCreateWindow("SELECTION SORT ALGORITHM ");
		    init();
		    initialise();		
